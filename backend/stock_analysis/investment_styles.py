...
{{
  ...
  "analysis_summary": "用巴菲特的语气，像和老朋友聊天一样，自然地分析这家公司。如果推荐买入，可以提一句'如果你想降低风险，可以考虑卖出看涨期权来收取权利金'这样的建议。语言要口语化、接地气（150字以内）",
  "strategy": "基于价值投资原则的具体操作建议。如果有期权策略推荐，自然地融入进去，比如'买入股票后，可以卖出虚值看涨期权增加收益'。语言要像和投资者对话（100字以内）"
}}
...
{{
  ...
  "analysis_summary": "用林奇轻松幽默的风格，像在和邻居聊天一样分析这只股票。如果看好成长，可以说'这公司有十倍股潜力，如果担心波动，买点看涨期权也不错'。语言要轻松、有趣（150字以内）",
  "strategy": "基于成长股投资的具体操作建议。可以自然地提到期权策略，比如'分批建仓，或者用看涨期权以小博大'。语言要像和朋友聊天（100字以内）"
}}
...
{{
  ...
  "analysis_summary": "用索罗斯的风格，像和专业交易员对话一样分析市场。如果趋势强劲，可以说'这波趋势刚起来，买看涨期权杠杆更高'。语言要犀利、直接（150字以内）",
  "strategy": "具体的交易策略：入场点、止损点、目标位、仓位管理。可以提到期权策略，比如'用期权做方向性交易，控制风险'。语言要专业但不啰嗦（100字以内）"
}}
